...
var commuteMapSlice = function commuteMapSlice(start, end, mapStream) {
  return Map.create(mapStream.f, slice$1(start, end, mapStream.source));
};
...
var aClicks = constant$$1('A', click(qs('[name=a]')));
var bClicks = constant$$1('B', click(qs('[name=b]')));
var value = qs('.value');
...
var results =
  compose(tap$$1(function (result) { value.innerText = render(result); }),
  compose(map$1(verify(isMatch)),
  compose(map$1(codeAndTime),
  compose(slidingWindow(6),
    withTime))));

runEffects$$1(results(merge$$1(aClicks, bClicks)), newDefaultScheduler$1());
